...
class PyBulletDeepMimicEnvRunTarget(Env):
  ...
  def reset(self):
    ...
    if not self._isInitialized:
      ...
      self._target_id = self._pybullet_client.createVisualShape(self._pybullet_client.GEOM_SPHERE,
                                                                radius=0.1,
                                                                rgbaColor=[0, 1, 0, 1])
      self._target_multi_body = self._pybullet_client.createMultiBody(baseMass=0,
                                                                      baseVisualShapeIndex=self._target_id,
                                                                      basePosition=[0, 0, 0])
  ...
  def calc_reward(self, agent_id):
    ...
    reward = self._humanoid.getReward(kinPose, self.target_direction)
    return reward
  ...
  def update(self, timeStep):
    ...
    for i in range(1):
      ...
      # Check if need to change target direction
      target_i = self.calculate_target_i()
      if target_i != self.target_i:
        self.target_i = target_i
        self.reset_target()

      self.update_target_indicator()
